...
**Tech Stack:** Cloudflare Workers, Hono, @modelcontextprotocol/sdk, agents, Drizzle ORM, D1, Zod 4. No server-side AI — the player's MCP client (Claude Code, Cursor, Codex) generates narratives and ASCII art.
...
| `wrangler.jsonc` | Worker config: D1, KV bindings (no AI) |
...
| `src/lib/mcp/tools/adopt.ts` | `adopt` tool — create pet with name, personality traits (returns structured data for client LLM) |
| `src/lib/mcp/tools/feed.ts` | `feed` tool — run Life Engine, return structured events + saved narratives, collect items |
| `src/lib/mcp/tools/pack.ts` | `pack` tool — manage travel pack |
| `src/lib/mcp/tools/items.ts` | `items` tool — view inventory |
| `src/lib/mcp/tools/save-story.ts` | `save-story` tool — persist client-generated narratives and ASCII art |
...
export const pet = sqliteTable("pet", {
  ...
  asciiArt: text("ascii_art"), // nullable — client LLM generates via save-story
  ...
  server.registerTool(
    ...
    async ({ name }) => {
      try {
        ...
        // No ASCII art yet — client LLM generates it, then calls save-story
...
## Task 7: MCP Tools — `save-story`

**Files:**
- Create: `workers/pebble/src/lib/mcp/tools/save-story.ts`

- [ ] **Step 1: Create `workers/pebble/src/lib/mcp/tools/save-story.ts`**
  ...
  server.registerTool(
    ...
    async ({ stories, petId, asciiArt }) => {
      try {
        ...
        if (pets.length === 0) {
          return {
            content: [{ type: "text" as const, text: "You don't have a pet yet!" }],
            isError: true as const,
          };
        }
        ...
        if (asciiArt && petId) {
          if (petId !== ownedPetId) {
            return {
              content: [{ type: "text" as const, text: "That's not your pet." }],
              isError: true as const,
            };
          }
          await db
        ...
        // Save story narratives
        if (stories && stories.length > 0) {
          for (const s of stories) {
            await db
              .update(storyTable)
              .set({ story: s.narrative })
              .where(
                and(
                  eq(storyTable.id, s.id),
                  eq(storyTable.petId, ownedPetId)
                )
              );
          }
        ...
        if (savedCount === 0) {
          return {
            content: [{ type: "text" as const, text: "Nothing to save. Provide stories or asciiArt." }],
          };
        }
...
```bash
git add workers/pebble/src/lib/mcp/tools/save-story.ts
git commit -m "feat(pebble): add save-story MCP tool for client-generated content"
```
  ...
  server.registerTool(
    ...
    async () => {
      try {
        ...
        // Return structured data — client LLM narrates new events
        const events = recentStories.reverse().map((s) => ({
          id: s.id,
          activityType: s.activityType,
          location: s.location,
          encounteredPetId: s.encounteredPetId,
          itemsFound: s.itemsFound ? JSON.parse(s.itemsFound) : null,
          narrative: s.story, // null for new events, saved text for old ones
          timeWindow: s.timeWindow,
        }));
...
function createServer(env: CloudflareBindings) {
  ...
  registerSaveStoryTool(server, env);
...
1. `adopt` — create a pet, verify traits in JSON response
2. `save-story` — save the client-generated ASCII art for the pet
3. `items` — see starter items
4. `pack` — add a rice ball to the pack
5. `feed` — wait a few minutes, then check — should see home activities as structured JSON
6. `save-story` — persist the client-generated narratives for new events
7. `feed` again — verify saved narratives appear in the response
